...
% function results = tcdExperiment02(fnameV, fnameS)
% Note that this is my attemp to bring everything to the World (IMU) frame
% but for some reason I can't transform the Vicon pos to World frame
...
%         dataV.(val{i}) = quatrotate(calibIR.(key{i}).ori, dataV.(val{i}));
        R = quat2rotm(calibIR.(key{i}).ori);
        dataV.(val{i}) = (R*(dataV.(val{i}))')';
%         v2 = strcat('q', val{i});
...
%                               quatconj(calibIB.R_LowLeg.ori));
    ...
    actBody = dataV.togrBody(1:4114, {'name', 'act', 'oriUnit', 'deg', ...
                             'lnSymbol', '-', 'ptSymbol', '.', ...
                             'xyzColor', {'m', 'y', 'c'}});
