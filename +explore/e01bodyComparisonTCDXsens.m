...
% Constraint (FEM z dot TIB y)
LFEM_z = estBody.LFEP - estBody.LFEO;
RFEM_z = estBody.RFEP - estBody.RFEO;
LTIB_y = quat2rotm(estBody.qLSK); LTIB_y = squeeze(LTIB_y(:,2,:))';
RTIB_y = quat2rotm(estBody.qRSK); RTIB_y = squeeze(RTIB_y(:,2,:))';
t = 1:estBody.nSamples;
plot(t, sum(LFEM_z.*LTIB_y, 2), t, sum(RFEM_z.*RTIB_y, 2));
