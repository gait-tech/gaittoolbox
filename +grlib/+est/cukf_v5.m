%cukf_v5 implemented knee pinjoint constraint on SR-UKF with nonlin
%projection of estimated x onto constrained space
function [ x_rec, xa_rec, qFEM ] = cukf_v5(x,P,Q,R,N_MP,nMeas,acc,fs,...
...
hjc = 3;%or 3 - see hingeJoint_cosntr func. for details
...
%     x1 = fmincon(@(x) L2Dist(x,x1),x1,[],[],[],[],[],[],@(x) hingeJoint_constrNL(x,1,q_MP, q_LA, q_RA,...
%     d_pelvis, d_lfemur, d_rfemur, d_ltibia, d_rtibia),options);
% 
% %         [x1] = hingeJoint_constrNL(x1,hjc,P,k,q_MP, q_LA, q_RA,...
...
function [c, ceq] = hingeJoint_constrNL(xhat,n,q_MP, q_LA, q_RA,...
    d_pelvis, d_lfemur, d_rfemur, d_ltibia, d_rtibia)
%   if hingejoint_constraint > 0 && hingejoint_constraint <= 8
% calculate the location of the knee
idx_pos_MP = [1:3]';
