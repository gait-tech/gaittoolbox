function dumpStepParams(obj1, obj2, intervals, fname)
    ...
    for i=posFields
        ...
        T.("Start Time (s)") = intervals2.(i)(1:end-1)/obj1.fs;
        T.("End Time (s)") = intervals2.(i)(2:end)/obj1.fs;
        T.("Duration (s)") = time;
        ...
        T.("Stride Length Est (m)") = dist1;
        T.("Gait Speed Est (m/s)") = dist1./time;
        ...
        % step length
        T.("Stride Length Act (m)") = dist2;
        % gait speed SL
        T.("Gait Speed Act (m/s)") = dist2./time;
