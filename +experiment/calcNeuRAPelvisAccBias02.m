% ======================================================================
%> @brief Calculate pelvis accelerometer bias from vicon data
%> @author Luke Sy
%>
...
% ======================================================================
function bias = calcNeuRAPelvisAccBias02(dataS, dataV, ...
    ...
    if ~isempty(dataV) & ~isempty(calibV2W)
        ...
        accMPRef = quatrotate(W__dataS.Pelvis.ori(idx,:), ...
                              acc.MIDPEL(idx,:) + [0 0 9.81]);
        
        % gfrAcc from sparse
        accMPEst = W__dataS.Pelvis.acc(idx,:);
        bias.w__v = mean(accMPEst - accMPRef, 1);
    end
    ...
    if ~isempty(dataV) & ~isempty(calibW2V)
        ...
        accMPRef = quatrotate(q, ...
                              acc.MIDPEL(idx,:) + [0 0 9.81]);
                          
        % gfrAcc from sparse
        accMPEst = W__dataS.Pelvis.acc(idx,:);
        
        bias.v__v = mean(accMPEst - accMPRef, 1);
    end
    ...
    if ~isempty(dataX)
        ...
        % gfrAcc from sparse
        accMPEst = W__dataS.Pelvis.acc(idx,:);
        bias.w__x = mean(accMPEst - accMPRef, 1);
    end
